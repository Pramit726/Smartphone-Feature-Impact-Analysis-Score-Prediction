...
- Orchestration: Orchestrated a scalable predictive assessment engine on a Kubernetes cluster, implementing Horizontal Pod Autoscaling (HPA) to handle concurrent RESTful analysis requests via FastAPI.
- Software Quality: Implemented a robust Unit Testing suite using Pytest and Unittest.mock, ensuring 100% logic coverage for data conversion and prediction pipelines.
...
The system is designed to handle high-concurrency environments by moving beyond simple containerization into full Kubernetes orchestration.

- **Kubernetes (K8s):** Managed deployment via Deployment and Service manifests for high availability  
- **Horizontal Pod Autoscaling (HPA):** CPU-based scaling implemented. When the prediction engine experiences a spike (e.g., >50% CPU utilization), the cluster automatically scales from 2 to 8 replicas  
- **Resource Management:** Defined strict requests and limits for CPU/Memory to prevent "Noisy Neighbor" effects and ensure stable inference latency  
...
| Autoscaling       | K8s HPA (Metrics Server)     |
| Containerization  | Docker                       |
| CI/CD             | GitHub Actions               |
...
To ensure the reliability of the prediction module, a comprehensive unit testing framework was implemented using **Pytest**.

### Testing Strategy

- **Isolation:**  
  Used `unittest.mock` to simulate model dependencies, allowing tests to run without loading heavy `.pkl` files or requiring external MLflow connections  

- **Data Integrity:**  
  Verified data conversion to ensure dictionary-to-dataframe mapping preserves data types and feature counts  
...
def test_predictor_predict_logic(mock_model_class, sample_input_data):
    mock_model_instance = mock_model_class.return_value
    mock_model_instance.predict.return_value = 4.5 
